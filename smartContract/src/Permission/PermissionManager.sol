...
contract PermissionManager is IPermissionManager, OwnableUpgradeable, PausableUpgradeable {
    // Storage
    ...
    modifier onlyWithPermission(bytes4 functionSelector) {
        require(_hasPermission(msg.sender, functionSelector), "Permission denied");
        _;
    }
    ...
    function _hasPermission(address account, bytes4 functionSelector) internal view returns (bool) {
        Permission memory permission = _permissions[account][functionSelector];
        if (!permission.isActive) {
            return false;
        }
